package com.example.theteacher.NeedToStreaming.encoder.input.video;
...
import com.example.theteacher.LecturePlayActivity;
import com.example.theteacher.NeedToStreaming.encoder.utils.YUVUtil;
...
//    YuvImage img = new YuvImage(data, imageFormat, surfaceView.getWidth(), surfaceView.getHeight(), null);
//    surfaceView.setZOrderOnTop(false);
//    ByteArrayOutputStream baos = new ByteArrayOutputStream();
//    Rect area = new Rect(0, 0, surfaceView.getWidth(), surfaceView.getHeight());
//
//    img.compressToJpeg(area, 100, baos);
//    Bitmap tmpBitmap = BitmapFactory.decodeByteArray(baos.toByteArray(), 0, baos.size());
//    LecturePlayActivity.sendBitmap = Bitmap.createScaledBitmap(tmpBitmap, 120,140, false);
    // SurfaceView에 띄워진 Preview를 capture하는 부분입니다.
    // NV21 포맷을 RGB 포멧으로 바꾼 후 bitmap에 저장합니다.
    LecturePlayActivity.sendBitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
    Allocation bmData = renderScriptNV21ToRGBA888(surfaceView.getContext(), width, height, data);
    bmData.copyTo(LecturePlayActivity.sendBitmap);
